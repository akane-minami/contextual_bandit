...
"""特徴量に応じたバンディットを定義、データをフィード→特徴量をサンプリング→特定の行動に対する報酬を与える→与えられた特徴量に対して最適な行動を与える"""
...
class ContextualBandit(object):
  ...
  def feed_data(self, data):#2000回終わったらデータを取り直す、それの初期化的な役割
    """1エピソード毎の初期化

    Args:
      data: [特徴数、特徴量の次元数+報酬(行動)の数]のnumpy配列
    Raises:
    ...
    self.data = data
    self.order = range(self.number_contexts) # 0からnumber_contextsまでの連番
  ...
  def optimal(self, number):
    ...
    Args:
      number(int):指定のstep数
    Returns:
      np.argmax(self.data[self.order[number]][self.context_dim:]):食べた時/食べなかった時の報酬
    """
